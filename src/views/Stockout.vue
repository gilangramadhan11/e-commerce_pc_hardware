<template>
  <div class="pr-8 pl-8 pt-8 pb-0">
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-3xl font-bold text-gray-800 mb-4">Stock Out</h1>
        <p class="text-gray-600 mt-1">Monitor and manage stock out product</p>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <template v-else-if="filteredStockOutHistory.length > 0">
                <tr v-for="item in filteredStockOutHistory" :key="item.id" class="hover:bg-gray-50">
                ...
                <tr>
                  <td colspan="7" class="px-6 py-8 text-center">
                    <div class="flex flex-col items-center justify-center">
                      ...
                      <p class="text-gray-600 font-medium">No stock out records yet</p>
                      <p class="text-gray-500 text-sm mt-1">Click "Add Stock Out" button to start recording</p>
                    </div>
                  </td>
                </tr>
              </template>
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="text-lg font-bold text-gray-800">
                  {{ isViewMode ? 'View Stock Out' : (isEditing ? 'Edit Stock Out' : 'Add Stock Out') }}
                </h3>
                <p class="text-sm text-gray-500">
                  {{ isViewMode ? 'Read-only stock details' : 'Record outgoing inventory' }}
                </p>
              </div>
            </div>
          ...
          <div class="p-6 space-y-5">
            ...
            <div v-if="selectedProduct" class="bg-gray-50 rounded-xl p-4 flex items-center gap-4 border border-gray-200">
              ...
              <div class="text-right">
                <p class="text-xs text-gray-500 mb-1">{{ isViewMode ? 'Removing Stock' : 'Current Stock' }}</p>
            ...
            <div v-if="!isViewMode && selectedProduct && form.quantity" class="grid grid-cols-3 gap-3">
              ...
              <div class="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-500 mb-1">Reduce</p>
                <p class="text-2xl font-bold text-red-600">-{{ form.quantity }}</p>
              </div>
              <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-500 mb-1">New Stock</p>
                <p :class="['text-2xl font-bold', selectedProduct.stock - (form.quantity || 0) < 0 ? 'text-red-600' : 'text-blue-600']">
                  {{ selectedProduct.stock - (form.quantity || 0) }}
                </p>
              </div>
            </div>
            ...
            <p v-if="form.quantity > selectedProduct?.stock" class="text-red-500 text-sm">
              Maximum available stock: {{ selectedProduct?.stock }}
            </p>
            <!-- Total Cost Preview -->
          ...
          <div class="flex gap-3 p-6 border-t bg-gray-50 rounded-b-xl">
            <button
              v-if="!isViewMode"
              @click="submitForm"
              :disabled="saving || !form.product_id || !form.quantity || form.quantity < 1 || isViewMode || !selectedProduct || form.quantity <= 0 || form.quantity > selectedProduct.stock"
            ...
            >
              ...
              {{ saving ? 'Saving...' : (isEditing ? 'Save Changes' : 'Confirm Stock Out') }}
            </button>
            <button
              @click="closeModal"
...
<script setup>
    ...
    try {
      ...
      if (isEditing.value) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Updated!',
          text: 'Stock out record has been updated successfully',
          confirmButtonColor: '#3B82F6'
        })
      } else {
        // Insert new stock out record
        if (form.quantity > selectedProduct.value.stock) {
          Swal.fire({
            icon: 'error',
            title: 'Insufficient Stock',
            text: `Cannot reduce ${form.quantity} units. Only ${selectedProduct.value.stock} units available in stock.`,
            confirmButtonColor: '#3B82F6'
          })
          return
        }
        ...
        Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: 'Stock out has been recorded successfully',
          confirmButtonColor: '#3B82F6'
        })
      }
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.message || 'Failed to save stock out record. Please try again.',
        confirmButtonColor: '#3B82F6'
      })
    } finally {
      saving.value = false
  ...
  const filteredStockOutHistory = computed(() => {
    let result = [...stockOutHistory.value]
  ...
  const deleteStockOut = async (item) => {
    const result = await Swal.fire({
      icon: 'warning',
      title: 'Delete Stock Out Record',
      text: `Are you sure you want to delete the stock out record for ${item.products?.name}?`,
    ...
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Deleted!',
        text: 'Stock out record has been deleted successfully',
        confirmButtonColor: '#3B82F6'
      })
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.message || 'Failed to delete stock out record. Please try again.',
        confirmButtonColor: '#3B82F6'
      })
    }
  }
